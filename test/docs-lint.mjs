// LS-24a dev/01 — docs-lint: asserts the four newer packages' READMEs carry
// the two canonical headings (`## Charter`, `## Security posture`) and each
// package's required security keywords, and that the root README names all
// four packages. Node built-ins only — no dependency on anything the repo
...
function assertKeywords(pkg, text, keywords) {
  ...
  for (const { label, re } of keywords) {
    check(`${pkg}/README.md security posture mentions ${label}`, re.test(text), `pattern ${re} not found anywhere in ${pkg}/README.md`);
  }
}

// ── lucarne-widget: CSP-bypass + build-time composition is the unit of trust ──
{
  const pkg = "lucarne-widget";
...
// ── lucarne-interact: the three safety-law mechanisms + zero-synthetic-requests capture law ──
{
...
// ── lucarne-records: dependency-free (so recorder + read bin share it with no inherited deps) ──
{
  const pkg = "lucarne-records";
  const text = readReadme(pkg);
  assertHeadings(pkg, text);
  assertKeywords(pkg, text, [{ label: "dependency-free", re: /dependency-free/i }]);
}

// ── lucarne-corpus-mcp: read-only, no capture — a miss says "not captured", never fetches ──
{
  const pkg = "lucarne-corpus-mcp";
  const text = readReadme(pkg);
  assertHeadings(pkg, text);
  assertKeywords(pkg, text, [
    { label: "read-only", re: /read-only/i },
    { label: "no-egress or not-captured", re: /no-egress|not[ _]captured/i },
  ]);
}

// ── root README names all four new packages ──
{
  ...
  if (exists) {
    ...
    for (const pkg of ["lucarne-records", "lucarne-interact", "lucarne-widget", "lucarne-corpus-mcp"]) {
      check(`root README.md names ${pkg}`, text.includes(pkg), `"${pkg}" not found anywhere in root README.md`);
    }
  }
}
