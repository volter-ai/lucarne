...
// must not skip), and runner-INDEPENDENT: it uses git + gh, which behave identically however the agent was
// launched. The RUNNER only provides the credential + env;
...
//   REVIEW_WORKFLOW  the reviewer's workflow to dispatch on github (empty if no review edge)
...
// itself: REVIEW_WORKFLOW names the reviewer's workflow and this dispatches it, the code-host realization
// of develop's `review:` edge.
