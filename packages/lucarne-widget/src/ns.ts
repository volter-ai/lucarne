...
// consumer-supplied namespace `ns` (e.g. "cadence"). Ported concept from cadence's `widget.ts` / `widget-bridge.ts`,
// which hard-coded a single `__cadence*` family of names (`widget.ts:39,190,197,241`, `widget-bridge.ts:36,159,255`,
// `main.tsx:17`) — see CADENCE-SPLIT-TASKSPEC.md LS-17: the FULL sweep that rewrites every remaining literal is a
// dedicated later commit, but every NAME THIS PACKAGE MINTS is `ns`-derived from day one, so that sweep has nothing
// left to do inside `lucarne-widget` itself.
//
...
/** The prefix shared by every `ns`-scoped page global this package mints, e.g. `__lw_cadence`. */
export function nsPrefix(ns: string): string {
  return `__lw_${assertNs(ns)}`;
}

/** The shadow-host element id mounted into the page (`widget.ts:39`'s `__cadence_widget_host`, generalized). */
 ...
 * `widget.ts`'s bare `d.__cadence` marker, `widget.ts:190`). Every message the iframe posts up to the top frame is
...
/** The window global holding one named intent queue (`onIntent(name, cb)` / `sendIntent(name, payload)`), e.g. `window.__lw_cadence_intent_ctl` (generalizes `widget-bridge.ts`'s `__cadenceCtl` / `__cadenceCfg`). */
export function intentQueueGlobal(ns: string, name: string): string {
  return `${nsPrefix(ns)}_intent_${name}`;
}

/** The window global holding the current page-probed theme (`widget.ts:69`'s `window.__cadenceTheme`). */
export function themeGlobal(ns: string): string {
  return `${nsPrefix(ns)}_theme`;
}

/** The window global holding the mounted iframe element reference (`widget.ts`'s `window.__cadenceIframe`). */
export function iframeGlobal(ns: string): string {
  return `${nsPrefix(ns)}_iframe`;
}

/** The window global persisting a dragged/snapped position across re-mounts (`window.__cadencePos`). */
export function posGlobal(ns: string): string {
  return `${nsPrefix(ns)}_pos`;
}

/** The window global holding the re-mount-guard interval id (`window.__cadenceGuard`). */
export function guardGlobal(ns: string): string {
  return `${nsPrefix(ns)}_guard`;
}

/** The window global holding in-flight drag state (`window.__cadenceDrag`). */
export function dragGlobal(ns: string): string {
  return `${nsPrefix(ns)}_drag`;
}

/** The window global holding the shadow-DOM scrim node reference (`window.__cadenceScrim`). */
...
/** The SVG refraction filter element ids (`__cadence_glass_svg`/`_glass`/`_glass_img`/`_glass_dm`). */
...
/** The durable sticky-injection id registered with the engine's `POST /sessions/:id/inject` (LS-02) — one shell registration per `ns`, e.g. `cadence-widget`. */
