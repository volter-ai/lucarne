...
// the origin app's `recall.ts:178-185`: keep only records whose id was reported as meaningfully on-screen
...
/** Extract the thread-root id from a page url (the origin app's `recall.ts:183`: `/status/(\d+)` on the
