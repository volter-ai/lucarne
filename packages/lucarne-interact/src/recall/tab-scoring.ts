// Active-tab selection + change classification — the PURE decision halves of the origin app's `SIG`
...
/** One tab's raw visibility signal (the origin app's per-page `p.evaluate(() => ({vis, foc, skip}))`). */
...
/**
 * Score every non-skipped candidate (`vis*2 + foc*1 + tieBreakBonus`, the origin app's `recall.ts:78`
 ...
 * the same as the origin app's `catch(e){ continue; }`, `recall.ts:73`).
 *
 * The `< 1` FALLBACK THRESHOLD is the caller's job, not this function's: the origin app falls back to the
...
/** The active-tab tie-break/fallback threshold (the origin app's `best < 1`, `recall.ts:84`). */
export const ACTIVE_TAB_FALLBACK_THRESHOLD = 1;

/** The parts of a capture-on-change signature the origin app tracks between ticks (`recall.ts:386-387`). */
...
/** the origin app's `shortUrl` (`recall.ts:391`) — a compact, readable URL for a `detail` string. */
 ...
 * — the origin app's `recall.ts:391-398`, ported verbatim. `prev === null` is the very first capture
...
/** the origin app's scroll bucketing (`recall.ts:386`: `Math.round(scrollY / 400)`). */
export function scrollBucket(scrollY: number): number {
  return Math.round(scrollY / 400);
}

/** the origin app's capture-on-change signature string (`recall.ts:387`), for the `s !== lastSig` gate. */
