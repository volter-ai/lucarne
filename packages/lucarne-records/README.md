...
- **`store.ts`** — `appendRecords`/`loadRecords`: an on-disk JSONL store,
  ...
  - **stub-never-degrades** — a record with real content (non-empty text/bio,
    or any real metric value) is never overwritten by a stub/empty one. The
    schema doesn't (yet) carry an explicit `stub` flag — that's
    `unitToRecord`'s job (LS-04) — so "stub-like" is derived structurally:
    empty text/bio AND no real metric.
...
file inside it: `<dir>/records.jsonl` — one JSON-encoded `Entity` per line, one
line per distinct `source:kind:id` identity (not one line per capture — merges
happen in place). Any other file in that directory is not this package's
concern.
